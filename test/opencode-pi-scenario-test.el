...
;; handler -> opencode-pi--route-event -> synthetic SSE events -> chat handlers
;; (on-session-status / on-part-updated / on-session-idle) -> buffer render.
  ...
  (let ((opencode-pi-scenario-test--filter nil)
    ...
    (unwind-protect
        (cl-letf (((symbol-function 'make-process)
                  ...
                  ;; Route synthetic SSE events straight to our chat buffer.
                  ((symbol-function 'opencode--dispatch-to-chat-buffer)
                   (lambda (_sid handler event)
                     (with-current-buffer chat-buf (funcall handler event))))
